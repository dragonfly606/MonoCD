...
def show_all_image_with_boxes(image, outputs, target, visualize_preds, vis_scores, depth_methods):
	...
	for i in range(num_gt):
		print('{}:gt_depth:{:.2f}, gt_height:{:.2f}'.format(i + 1, gt_locs[i, -1], gt_boxes[i, 3] - gt_boxes[i, 1]))

	for output, visualize_pred, vis_score, depth_method in zip(outputs, visualize_preds, vis_scores, depth_methods):
		output = output.cpu().float().numpy()

		if vis_score is not None:
			output[:, -1] = vis_score.squeeze().cpu().float().numpy()
		...
		proj_center = visualize_pred['proj_center'].cpu()
		keypoints = visualize_pred['keypoints'].cpu()



		pred_heatmap = visualize_pred['heat_map']
	...
	for i in range(len(stack_imgs)):
		plt.subplot(len(stack_imgs), 1, i+1)
		plt.imshow(stack_imgs[i])
		plt.title(depth_methods[i])
		plt.axis('off')
...
def show_image_with_boxes(image, output, target, visualize_preds, vis_scores=None, depth_method=None):
	...
	plt.subplot(212)
	if not depth_method == None:
		plt.imshow(stack_img); plt.title(depth_method); plt.axis('off')
	else:
